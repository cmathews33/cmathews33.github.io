...
import { ApiService, HistoryPeriod, TickerHistory } from './api.service';

export type { HistoryPeriod };

export interface PeriodState {
  readonly data: TickerHistory[];
  readonly loading: boolean;
  readonly error: string | null;
  readonly refreshedAt: Date | null;
}

const INITIAL_STATE: PeriodState = { data: [], loading: false, error: null, refreshedAt: null };
...
export class HistoryService {
  ...
  load(period: HistoryPeriod): void {
    ...
    if (s().loading || s().data.length > 0) return;

    s.update(st => ({ ...st, loading: true, error: null }));
    this.api.getHistorical(period)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: data => s.set({ data, loading: false, error: null, refreshedAt: new Date() }),
        error: () =>
